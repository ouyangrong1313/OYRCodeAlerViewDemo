...
- (void)layoutSubviews//属性设置完以后就在这里再布局界面
{
    [super layoutSubviews];
    
    _tipLable = [[UILabel alloc]initWithFrame:CGRectMake(0,5,270,43)];
    ...
    _tipLable.font = [UIFont fontWithName:@"Arial-BoldItalicMT" size:18];
...
- (void)creatViewWithAlert
{
    ...
    */
    _pzxView = [[PZXVerificationCodeView alloc]initWithFrame:CGRectMake(15, _tipLable.frame.origin.y+20+ _tipLable.frame.size.height - 10, self.alertview.frame.size.width-30, 40)];
    _pzxView.selectedColor = [UIColor colorWithRed:128/255.0 green:128/255.0 blue:128/255.0 alpha:1.0];
    //    _pzxView.center = self.view.center;
    _pzxView.deselectColor = [UIColor colorWithRed:128/255.0 green:128/255.0 blue:128/255.0 alpha:1.0];
    _pzxView.VerificationCodeNum = 4;
    //    _pzxView.isSecure = YES;
    _pzxView.Spacing = 20;//每个格子间距属性
    [self.alertview addSubview:_pzxView];
...
- (void)createBtnTitle:(NSArray *)titleArr
{
    ...
    CGFloat m = self.alertview.frame.size.width;//总长度
    
    UIView * line1 = [[UIView alloc]initWithFrame:CGRectMake(0, self.alertview.frame.size.height - 48, self.alertview.frame.size.width, 0.5)];
    line1.backgroundColor = [UIColor colorWithRed:210/255.0 green:210/255.0 blue:210/255.0 alpha:1.0];
    [self.alertview addSubview:line1];

    UIView * line2 = [[UIView alloc]initWithFrame:CGRectMake(self.alertview.frame.size.width/2, self.alertview.frame.size.height - 48, 0.5, 48)];
    line2.backgroundColor = [UIColor colorWithRed:210/255.0 green:210/255.0 blue:210/255.0 alpha:1.0];
    ...
    for (int i=0; i<_numBtn; i++) {
        ...
        if (_numBtn == 1) {
            btn.frame = CGRectMake(20, self.alertview.frame.size.height-48 + 5,(m-40), 33);
        }else{
            
            btn.frame = CGRectMake(20+i*(20+(m-60)/2), self.alertview.frame.size.height-48 + 5, (m-60)/2, 33);
        }
